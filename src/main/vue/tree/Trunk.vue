<template>
    <transition name="fade" v-if="trunk">
        <v-card>
            <v-toolbar>
                <v-toolbar-title style="width:100%">
                    <span v-if="!name.editing" @click="activateRenaming">{{trunk.name.toUpperCase()}}</span>
                    <v-layout row v-else style="margin-left: 1em">
                        <v-text-field ref="renamingInput" :value="trunk.name" @input="name.input = $event" @keydown.esc.native="unactivateRemaning" @keydown.enter.native="validateRenaming"/>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions([On.UPSERT_QUANTITY, On.RENAME_TRUNK]),
            activateRenaming() {
            ...
            validateQuantity() {
                this.upsertQuantity({trunk: this.trunk, quantity: this.qt.input});
